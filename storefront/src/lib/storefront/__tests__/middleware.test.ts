...
describe("Storefront locale & auth middleware", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("redirects an unsupported storefront locale without dropping the path", async () => {
    ...
    expect(response.cookies.get(MIRZA_COUNTRY_COOKIE)?.value).toBe("us");
    expect(response.cookies.get(MIRZA_LOCALE_COOKIE)?.value).toBe("en");
  });

  it("falls back to a supported locale when the configured default is unavailable", async () => {
    const invalidDefaultMiddleware = createStorefrontMiddleware({
      defaultCountry: "us",
      defaultLocale: "it",
  ...
  it.each([
    "/us/en/account/profile",
    "/us/en/account/orders",
    "/us/en/account/addresses",
  ])("always invokes verifyProxySession on protected account route: %s", async (pathname) => {
    await middleware(
      new NextRequest(`https://store.example${pathname}`),
    );
  ...
  it("does NOT authorize a protected account request with legacy auth cookies", async () => {
    const request = new NextRequest(
      "https://store.example/us/en/account/orders",
    );
    request.cookies.set(LEGACY_ACCESS_TOKEN_COOKIE, "legacy-token");
    ...
    // Defensively drops legacy auth cookies
