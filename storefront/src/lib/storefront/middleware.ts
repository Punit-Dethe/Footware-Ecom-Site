...
export interface StorefrontMiddlewareConfig {
...
/**
 * Set mirza_country / mirza_locale cookies on a response so that
 * `getLocaleOptions()` reads values matching the URL during SSR.
 * Also expires legacy country/locale cookies.
 */
function setLocaleCookies(
  ...
  response.cookies.set(MIRZA_COUNTRY_COOKIE, country, {
    path: "/",
    maxAge: COOKIE_MAX_AGE,
  });
  response.cookies.set(MIRZA_LOCALE_COOKIE, locale, {
    path: "/",
    maxAge: COOKIE_MAX_AGE,
  });
...
function nextWithLocaleContext(
  ...
  // Migrate legacy cart cookies seamlessly on next request
  migrateLegacyCartCookies(request, response);
...
export function createStorefrontMiddleware(
  config: StorefrontMiddlewareConfig = {},
  ...
  return async function middleware(request: NextRequest): Promise<NextResponse> {
    ...
    if (match) {
      ...
      if (isAuthConsumingRoute(pathname, canonicalPrefix)) {
        ...
        if (
          ...
          expireLegacyAuthCookies(redirectResponse);
    ...
    const country =
      resolveCountry(request.cookies) ??
    ...
    const cookieValue = resolveLocale(request.cookies);
    const cookieLocale =
      supportedLocales.length > 0
